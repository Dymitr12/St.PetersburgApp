...
class PostViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        imageViewUp.contentMode = .scaleAspectFit
        imageViewDown.contentMode = .scaleAspectFit
        ...
        let tapGestureUp = UITapGestureRecognizer(target: self, action: #selector(PostViewController.imageTappedUp(gesture:)))
        let tapGestureDown = UITapGestureRecognizer(target: self, action: #selector(PostViewController.imageTappedDown(gesture:)))
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        guard let rezultView = segue.destination as? ResultViewController else { return }
